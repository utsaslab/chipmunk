...
type Config struct {
	...
	// path to the shared directory that will be mounted on the VM
...
func (inst *instance) boot() error {
	...
	if inst.cfg.Kernel != "" {
		...
		sharedir := strings.TrimRight(inst.cfg.ShareDir, "/")
		sharedir = sharedir + "-" + strconv.Itoa(inst.index)
		args = append(args,
		...
		// if _, err := os.Stat(sharedir); errors.Is(err, os.ErrNotExist) {
		// 	log.Logf(0, "%v does not exist, creating", sharedir)
		err := os.MkdirAll(sharedir, os.ModePerm)
		if err != nil {
			return fmt.Errorf("failed to create directory %v: %v", sharedir, err)
		}
		// }

		err = os.MkdirAll(sharedir+"/logs/diffs", os.ModePerm)
		if err != nil {
			return fmt.Errorf("failed to create directory %v: %v", sharedir+"/logs/diffs", err)
		}
		err = os.MkdirAll(sharedir+"/logs/workloads", os.ModePerm)
		if err != nil {
			return fmt.Errorf("failed to create directory %v: %v", sharedir+"/logs/workloads", err)
		}
		err = os.MkdirAll(sharedir+"/crashConsistencyProgs", os.ModePerm)
		if err != nil {
			return fmt.Errorf("failed to create directory %v: %v", sharedir+"crashConsistencyProgs", err)
		}
		// }
	}
	// log.Logf(0, "INSTANCE INDEX %v", inst.index)
	if inst.debug {
...
// TODO: would probably be faster to copy the whole directory instead of file by file?
func (inst *instance) CopyDir(hostSrc, guestDst string) error {
	rpipe, wpipe, err := osutil.LongPipe()
	if err != nil {
		return err
	}
	inst.merger.Add("ssh", rpipe)
	log.Logf(0, "host src %v", hostSrc)
	command := "mkdir " + guestDst
	args := append(vmimpl.SCPArgs(inst.debug, inst.sshkey, inst.port), inst.sshuser+"@localhost", command)
	log.Logf(0, "running command: ssh %#v", args)

	ssh_cmd := osutil.Command("ssh", args...)
	ssh_cmd.Stdout = wpipe
	ssh_cmd.Stderr = wpipe
	if err := ssh_cmd.Start(); err != nil {
		wpipe.Close()
		return err
	}

	args = append(vmimpl.SCPArgs(inst.debug, inst.sshkey, inst.port), "-r", hostSrc, inst.sshuser+"@localhost:"+guestDst)
	log.Logf(0, "running command: scp %#v", args)
	_, err = osutil.RunCmd(10*time.Minute*inst.timeouts.Scale, "", "scp", args...)
	if err != nil {
		return err
	...
	// // now go through and copy all of the tests over
	// // TODO: you should have a way to run just one test I think. you could do that easily
	// // by JUST copying that file?
	// items, err := ioutil.ReadDir(hostSrc)
	// if err != nil {
	// 	return fmt.Errorf("error reading directory %v: %v", hostSrc, err)
	// }

	// for _, item := range items {
	// 	srcFile := filepath.Join(hostSrc, item.Name())
	// 	args = append(vmimpl.SCPArgs(inst.debug, inst.sshkey, inst.port), srcFile, inst.sshuser+"@localhost:"+guestDst)
	// 	log.Logf(0, "running command: scp %#v", args)
	// 	_, err := osutil.RunCmd(10*time.Minute*inst.timeouts.Scale, "", "scp", args...)
	// 	if err != nil {
	// 		return err
