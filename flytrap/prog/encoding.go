...
func (p *parser) parseProg() (*Prog, error) {
	...
	for p.Scan() {
		...
		if r == "checkpoint" { // here to read programs that contain checkpoints
			//log.Logf(0, "CHECKPOINT IN PARSING")
			//i, err := strconv.Atoi(name)
			//if err != nil {
			//	return nil, fmt.Errorf("unknown checkpoint: %v", name)
			//}
			//log.Logf(0, "CHECKPOINT: %v", prog.Checkpoint)
			continue
		}
...
func CallSet(data []byte) (map[string]struct{}, int, error) {
	...
	for len(data) > 0 {
		...
		if len(ln) == 0 || ln[0] == '#' || strings.Contains(string(ln), "checkpoint") {
			continue
		}
