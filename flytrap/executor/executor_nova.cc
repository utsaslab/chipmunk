...
static const std::string mount_point = "/mnt/pmem";
...
void execute_one()
{
    ...
    res = system("ls -l /mnt/pmem/");
    if (res < 0) {
        debug("FAILED TO LS: %s", strerror(errno));
    }
    cwd = getcwd_string();
    srand (time(NULL));
    test_sandbox = "test_" + std::to_string(rand());
    debug("TEST SANDBOX1111: %s\n", test_sandbox.c_str());
    dest = mount_point + "/" + test_sandbox;
    res = mkdir(dest.c_str(), 0777);
    if (res < 0) {
        debug("error making directory: %s\n", strerror(res));
    }
    ...
    debug("DELETING DIRECTORY: %s\n", (std::string("rm -rf ") + dest).c_str());
    res = system((std::string("rm -rf ") + dest).c_str());
    if (res < 0) {
        debug("Error removing dir: %s\n", strerror(errno));
    }
    res = chdir(cwd.c_str());
    if (res < 0) {
        debug("Error changing dir: %s\n", strerror(errno));
    }
}
...
void write_coverage_signal(cover_t* cov, uint32* signal_count_pos, uint32* cover_count_pos)
{
	...
	bool wrote_signal_output = false;
