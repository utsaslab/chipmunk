...
const (
	CrashConsistencyProgsDir = "/root/tmpdir/crashConsistencyProgs/"
)
...
type Config struct {
	...
	//Path to consistency check binary
	ExecutorCC string
	...
	CheckConsistency bool   // check fs consistency
	CollectMountCov  bool   // collect coverage from mounting crash states
	Reload           bool   // reload file system module after each test
	FS               string // file system to test
	Module           bool   // load file system as module
	FSLogger         string // path to logger module
	FSPath           string // path to file system module
...
func MakeEnv(config *Config, pid int) (*Env, error) {
	...
	var inf, outf, outfMmap *os.File
	var inmem, outmem, outmemMount []byte
	var binary string
	if config.UseShmem {
		var err error
		...
		if config.CheckConsistency {
			outfMmap, outmemMount, err = osutil.CreateMemMappedFile(outputMountSize)
			if err != nil {
				return nil, err
			}
			log.Logf(0, "OUTFMMAP: %v", int(outfMmap.Fd()))
			defer func() {
				if outfMmap != nil {
					osutil.CloseMemMappedFile(outfMmap, outmemMount)
				}
			}()
		}
	} else {
		...
		outmemMount = make([]byte, outputMountSize)
	}
	if config.CheckConsistency {
		binary = config.ExecutorCC
	} else {
		binary = config.Executor
	}
	log.Logf(0, "BINARY: %s", binary)
	env := &Env{
		in:           inmem,
		out:          outmem,
		outMount:     outmemMount,
		inFile:       inf,
		outFile:      outf,
		outFileMount: outfMmap,
...
func (env *Env) Close() error {
	...
	if env.outFileMount != nil {
		err3 = osutil.CloseMemMappedFile(env.outFileMount, env.outMount)
	}
	switch {
	case err1 != nil:
		return err1
...
func (env *Env) Exec(opts *ExecOpts, p *prog.Prog, name string) (output []byte, info *ProgInfo, hanged bool, err0 error) {
	// Copy-in serialized program.
	opts.ProgLen = len(p.Calls)
	...
	if env.cmd == nil {
		...
		log.Logf(1, "ABOUT TO MAKE COMMAND: %v", env.bin)
		env.cmd, err0 = makeCommand(env.pid, env.bin, env.config, env.inFile, env.outFile, env.outFileMount, env.out, env.outMount, tmpDirPath, name)
		log.Logf(1, "MADE COMMAND\n")
		if err0 != nil {
			if opts.Restart {
				env.cmd.close()
				env.cmd = nil
	...
	output, hanged, err0 = env.cmd.exec(opts, progData, p.Serialize(), p.Name)
	if err0 != nil {
		env.cmd.inrp.Close()
		env.cmd.outwp.Close()
		env.cmd.close()
	...
	//if env.config.CheckConsistency {
	//	log.Logf(0, "MOUNT OUT: %s", string(env.outMount[:100]))
	//}
	//log.Logf(0, "PARSE ERR: %v", err0)
	if info != nil && env.config.Flags&FlagSignal == 0 {
		addFallbackSignal(p, info)
	}
	if !env.config.UseForkServer || opts.Restart {
		env.cmd.close()
		env.cmd = nil
	}
	return
...
func (env *Env) parseOutput(p *prog.Prog) (*ProgInfo, error) {
	...
	for i := uint32(0); i < ncmd; i++ {
		...
		//log.Logf(0, "SIGNAL: %v", inf.Signal)
	...
	if env.config.CheckConsistency && env.config.CollectMountCov {
		outmemMount := env.outMount[4:]
		//mountSignal := make([]uint32, 0)
		//mountCover := make([]uint32, 0)
		//parseSig := true
		//for i := 0; i < 100; i++ {
		//	log.Logf(0, "outmem: %v", outmemMount[i])
		//}
		size := 0
		// we memset the mount buffer to 0 before
		for i := len(outmemMount) - 1; i > 0; i-- {
			if outmemMount[i] != 0 {
				size = i + 1
				break
			}
		}
		log.Logf(0, "SIZE: %v", size)
		//log.Logf(0, "%v", outmemMount[:size*4])
		cover, ok := readUint32Array(&outmemMount, uint32(size))
		if !ok {
			return nil, fmt.Errorf("Failed to parse mount outmem")
		}
		mountSignalPos := []uint32{0}
		mountCoverPos := make([]uint32, 0)
		for i := 0; i < len(cover); i++ {
			if cover[i] == 1 {
				log.Logf(0, "SIGNAL END")
				mountCoverPos = append(mountCoverPos, uint32(i))
			} else if cover[i] == 2 {
				log.Logf(0, "COVER END")
				mountSignalPos = append(mountSignalPos, uint32(i))
			}
		}
		log.Logf(0, "%v %v", mountSignalPos, mountCoverPos)
		if len(mountCoverPos) != len(mountSignalPos)-1 {
			// during the machine check no coverage is collected so we don't want to throw this failure then
			if len(mountSignalPos) != 1 || len(mountCoverPos) != 0 {
				return nil, fmt.Errorf("Number of cover positions is not equal to number of signal positions: %v %v", len(mountCoverPos), len(mountSignalPos))
			}
		}
		for i := 0; i < len(mountCoverPos); i++ {
			info.MountSignal = append(info.MountSignal, cover[mountSignalPos[i]+1:mountCoverPos[i]]...)
			if i == len(mountCoverPos)-1 {
				info.MountCover = append(info.MountCover, cover[mountCoverPos[i]+1:size]...)
			} else {
				info.MountCover = append(info.MountCover, cover[mountCoverPos[i]+1:mountSignalPos[i+1]]...)
			}
		}
		info.Extra.Signal = info.MountSignal
		info.Extra.Cover = info.MountCover
		log.Logf(0, "%v %v", len(info.MountSignal), len(info.MountCover))
...
func makeCommand(pid int, bin []string, config *Config, inFile, outFile *os.File, outfileMount *os.File, outmem []byte,
	outmemMount []byte, tmpDirPath string, name string) (*command, error) {
	...
	rp, wp, err := os.Pipe()
	if err != nil {
		log.Logf(0, "ERROR CREATING output pipe: %v", err)
	...
	if err != nil {
		log.Logf(0, "ERROR CREATING executor->ipc pipe: %v", err)
		return nil, fmt.Errorf("failed to create pipe: %v", err)
	}
	defer inwp.Close()
	c.inrp = inrp
	...
	if err != nil {
		log.Logf(0, "ERROR CREATING ipc->executor pipe: %v", err)
		return nil, fmt.Errorf("failed to create pipe: %v", err)
	}
	defer outrp.Close()
	c.outwp = outwp
	...
	log.Logf(0, "OUT WP: %d %d %d %d", c.outwp.Fd(), outrp, c.inrp.Fd(), inwp)
	if config.CheckConsistency {
		bin = append(bin, name)
		bin = append(bin, config.FSLogger)
		bin = append(bin, config.FS)
		bin = append(bin, config.FSPath)
	}
	if config.CollectMountCov && config.CheckConsistency {
		bin = append(bin, "mountCov")
	}

	if config.Module && config.CheckConsistency {
		bin = append(bin, "-module")
	}

	if config.Reload && config.CheckConsistency {
		bin = append(bin, "reload")
	}

	log.Logf(0, "BIN: %v\n", bin)
	cmd := osutil.Command(bin[0], bin[1:]...)
	if inFile != nil && outFile != nil {
		cmd.ExtraFiles = []*os.File{inFile, outFile, outfileMount}
	}
	cmd.Dir = dir
	...
	log.Logf(0, "STARTING COMMAND\n")
	if err := cmd.Start(); err != nil {
		return nil, fmt.Errorf("failed to start executor binary: %v", err)
	}
	log.Logf(0, "STARTED COMMANED\n")
	c.cmd = cmd
	wp.Close()
	...
	inwp.Close()
	log.Logf(0, "ABOUT TO HANDSHAKE")
	if c.config.UseForkServer {
		log.Logf(0, "USING FORK SERVER")
		if err := c.handshake(); err != nil {
			return nil, err
		}
	}
	log.Logf(0, "HANDSHOOK")
	tmp := c
...
func (c *command) handshake() error {
	...
	go func() {
		...
		if reply.magic != outMagic {
			read <- fmt.Errorf("bad handshake reply magic 0x%x, expected 0x%x", reply.magic, outMagic)
			return
		}
		read <- nil
	}()
...
func (c *command) exec(opts *ExecOpts, progData []byte, progSerialized []byte, testName string) (output []byte, hanged bool, err0 error) {
	req := &executeReq{
		magic:            inMagic,
		...
		progLen:          uint64(opts.ProgLen),
	}
	reqData := (*[unsafe.Sizeof(*req)]byte)(unsafe.Pointer(req))[:]
	log.Logf(0, "WRITING DATA: %v\n", req)
	if _, err := c.outwp.Write(reqData); err != nil {
		output = <-c.readDone
	...
	if opts.ConsistencyCheck {
		...
		name := ""
		if testName == "" {
			// if the test doesn't already have a name, get a timestamp to use as its name
			now := time.Now()
			nsec := now.UnixNano()
			name = strconv.FormatInt(nsec, 16)
		} else {
			name = testName
		}

		log.Logf(0, "%v", name)
		ioutil.WriteFile(filepath.Join(CrashConsistencyProgsDir,
			name), progSerialized, os.ModePerm)
		log.Logf(0, "WRITING NAME: %s\n", name)
		log.Logf(0, "%s", string(progSerialized[:]))
		if _, err := c.outwp.Write([]byte(name + "\x00")); err != nil {
			output = <-c.readDone
			err0 = fmt.Errorf("executor %v: failed to write control pipe: %v", c.pid, err)
			return
		}
	...
	hang := make(chan bool)
	log.Logf(0, "TIMEOUT DURATION: %v", c.timeout)
	go func() {
		t := time.NewTimer(c.timeout)
		select {
		case <-t.C:
			log.Logf(0, "KILLING PROCESS")
			c.cmd.Process.Kill()
			hang <- true
		case <-done:
			log.Logf(0, "DONE")
	...
	for {
		...
		log.Logf(0, "CALL REPLY DATA 0: %v", callReplyData)
		...
		log.Logf(0, "CALL REPLY DATA 1: %v", callReplyData)
