...
func (inst *inst) testInstance() error {
	...
	cmd := OldFuzzerCmd(fuzzerBin, executorBin, targets.TestOS, inst.cfg.TargetOS, inst.cfg.TargetArch, fwdAddr,
		inst.cfg.Sandbox, 0, inst.cfg.Cover, true, inst.optionalFlags, inst.cfg.Timeouts.Slowdown, inst.cfg.CrashConsistency,
		inst.cfg.CaptureMountCov, inst.cfg.LoggerPath, inst.cfg.FileSystem, inst.cfg.Module, inst.cfg.Restart, inst.cfg.Reload)
...
func FuzzerCmd(fuzzer, executor, name, OS, arch, fwdAddr, sandbox string, procs, verbosity int,
	cover, debug, test, runtest, optionalFlags bool, slowdown int, crashConsistency bool, captureMountCov bool,
	logger string, filesystem string, module bool, restart bool, reload bool, fsPath string) string {
	osArg := ""
	...
	return fmt.Sprintf("%v -executor=%v -name=%v -arch=%v%v -manager=%v -sandbox=%v"+
		" -procs=%v -cover=%v -debug=%v -test=%v%v%v%v -crashConsistency=%v -ccmount=%v"+
		" -logger=%v -restart=%v -module=%v -fs=%v -reload=%v -fs_path=%v",
		fuzzer, executor, name, arch, osArg, fwdAddr, sandbox,
		procs, cover, debug, test, runtestArg, verbosityArg, optionalArg, crashConsistency, captureMountCov, logger,
		restart, module, filesystem, reload, fsPath)
}

func OldFuzzerCmd(fuzzer, executor, name, OS, arch, fwdAddr, sandbox string, procs int,
	cover, test, optionalFlags bool, slowdown int, crashConsistency bool, captureMountCov bool,
	logger string, filesystem string, module bool, restart bool, reload bool) string {
	return FuzzerCmd(fuzzer, executor, name, OS, arch, fwdAddr, sandbox, procs, 0, cover, false, test, false,
		optionalFlags, slowdown, crashConsistency, captureMountCov, logger, filesystem, module, restart, reload, "")
}
