...
func (mgr *Manager) httpSummary(w http.ResponseWriter, r *http.Request) {
	...
	log.Logf(0, "EXECUTING TEMPLATE\n")
	executeTemplate(w, summaryTemplate, data)
	log.Logf(0, "EXECUTED TEMPLATE\n")
}
...
func (mgr *Manager) getNumberCrashStates() int {
	vmdirs := getVmDirs(mgr.cfg.TesterDir)
	states := 0
	for _, vmdir := range vmdirs {
		crashStateLog := filepath.Join(mgr.cfg.TesterDir, filepath.Join(vmdir, "crashStatesLog"))
		data, err := ioutil.ReadFile(crashStateLog)
		if err != nil {
			log.Logf(0, "Failed to read crash state log: %v", crashStateLog)
		}
		states += bytes.Count(data, []byte("\n"))
	}
	return states
}
...
func (mgr *Manager) collectStats() []UIStat {
	...
	if mgr.cfg.CrashConsistency {
		states := mgr.getNumberCrashStates()
		stats = append(stats, UIStat{Name: "crash states tested", Value: fmt.Sprint(states)})
	}
...
func (mgr *Manager) httpCrash(w http.ResponseWriter, r *http.Request) {
	...
	reg, err := regexp.Compile("diff-[0-9]+")
	if err != nil {
		log.Fatalf("Failed to compile regex: %v", err)
	}
	var crash *UICrashType
	if mgr.cfg.CrashConsistency && reg.Match([]byte(crashID)) {
		crash = mgr.CCBugManager.getCrash(crashID)
		log.Logf(0, "CRASH: %v", crash)
		if crash == nil {
...
func (mgr *Manager) httpFile(w http.ResponseWriter, r *http.Request) {
	...
	if mgr.cfg.CrashConsistency && strings.HasPrefix(file, mgr.cfg.TesterDir) {
		f, err := os.Open(file)
		if err != nil {
...
func (mgr *Manager) collectCrashes(workdir string) ([]*UICrashType, error) {
	...
	log.Logf(0, "COLLECTING CRASHES")
	reproReply := make(chan map[string]bool)
	if !mgr.cfg.DashboardOnly {
		mgr.reproRequest <- reproReply
		log.Logf(0, "REPRO REQUEST")
	}

	log.Logf(0, "EXECUTING COLLECT CRASHES")
	...
	if mgr.cfg.CrashConsistency {
		mgr.mu.Lock()
		mgr.stats.crashTypes.add(-1 * len(mgr.CCBugManager.uiCrashClusters))
		mgr.stats.crashes.add(-1 * mgr.CCBugManager.getCrashes())
		mgr.CCBugManager.updateClusters()
		mgr.stats.crashTypes.add(len(mgr.CCBugManager.uiCrashClusters))
		mgr.stats.crashes.add(mgr.CCBugManager.getCrashes())
		mgr.mu.Unlock()
		crashTypes = append(crashTypes, mgr.CCBugManager.uiCrashClusters...)
	}
...
<table class="list_table">
	...
	<tr>
		...
		<td>
			{{if $c.Tag}}
				<a href="/file?name={{$c.Tag}}">Program</a></td>
			{{end}}
		</td>
	</tr>
	{{end}}
