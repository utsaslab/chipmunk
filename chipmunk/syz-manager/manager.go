...
func main() {
	...
	if cfg.Reload && !cfg.Module {
		log.Fatalf("reload config requires module")
	}
	RunManager(cfg)
}
...
func RunManager(cfg *mgrconfig.Config) {
	...
	if mgr.cfg.CrashConsistency {
		mgr.mu.Lock()
		log.Logf(0, "tester dir: %v", mgr.cfg.TesterDir)
		mgr.CCBugManager = BugManagerCtor(mgr.cfg.TesterDir)
		mgr.stats.crashTypes.add(len(mgr.CCBugManager.uiCrashClusters))
		mgr.stats.crashes.set(mgr.CCBugManager.getCrashes())
		mgr.mu.Unlock()
	}

	if mgr.cfg.DashboardOnly {
		for {
...
func (mgr *Manager) runInstanceInner(index int, instanceName string) (*report.Report, error) {
	...
	_, err = inst.Copy(mgr.cfg.ExecutorCCBin)
	if err != nil {
		return nil, fmt.Errorf("failed to copy binary: %v", err)
	}
	...
	if mgr.cfg.CrashConsistency {
		_, err = inst.Copy(mgr.cfg.LoggerObj)
		if err != nil {
	...
	if mgr.cfg.Tester == "ace" {
		// Copy() copies into the root directory
		_, err = inst.Copy(mgr.cfg.AceTesterBin)
		if err != nil {
			return nil, fmt.Errorf("failed to copy binary: %v", err)
		}
		// also create a directory for the tests and copy them all into that
		err = inst.CopyDir(mgr.cfg.AceTestDir, "/ace_tests")
		if err != nil {
			return nil, fmt.Errorf("failed to copy binary: %v", err)
		}
	...
	log.Logf(0, "CRASH CONSISTENCY: %v, MOUNT COV: %v", mgr.cfg.CrashConsistency, mgr.cfg.CaptureMountCov)
	cmd := ""
	if mgr.cfg.Tester == "ace" {
		// TODO: we are not actually in a state where this will run successfully yet
		// right now, just want to confirm that it can start up. but it can't run any tests
		// yet.
		// TODO: should have a manager process, like the fuzzer here, that sets things up
		// (like loading the module) and runs the tests. that will make it easier to manage
		// test failures too
		// this does succeed in running the ace executor though
		cmd = "/ace-executor_cc -v -c /ace_tests/j-lang1.so"
	} else {
		// cmd = instance.FuzzerCmd(fuzzerBin, executorBin, instanceName,
		// 	mgr.cfg.TargetOS, mgr.cfg.TargetArch, fwdAddr, mgr.cfg.Sandbox, procs, fuzzerV,
		// 	mgr.cfg.Cover, *flagDebug, false, false, true, mgr.cfg.Timeouts.Slowdown, mgr.cfg.CrashConsistency,
		// 	mgr.cfg.CaptureMountCov, mgr.cfg.LoggerPath, mgr.cfg.FileSystem, mgr.cfg.Module,
		// 	mgr.cfg.Restart, mgr.cfg.Reload, "/nova.ko")
		cmd = instance.FuzzerCmd(fuzzerBin, executorBin, instanceName,
			mgr.cfg.TargetOS, mgr.cfg.TargetArch, fwdAddr, mgr.cfg.Sandbox, procs, fuzzerV,
			mgr.cfg.Cover, *flagDebug, false, false, true, mgr.cfg.Timeouts.Slowdown, mgr.cfg.CrashConsistency,
			mgr.cfg.CaptureMountCov, mgr.cfg.LoggerPath, mgr.cfg.FileSystem, mgr.cfg.Module,
			mgr.cfg.Restart, mgr.cfg.Reload, "/"+mgr.cfg.FileSystem+".ko")
		log.Logf(0, "%v", cmd)
	}
...
func (mgr *Manager) saveCover() {
	mgr.mu.Lock()
	defer mgr.mu.Unlock()
	log.Logf(0, "SAVING COVER")
	timestamp := time.Now().String()
	buf := make([]byte, 0)
	uniqueCovers := make(map[uint32]bool)
	for _, cov := range mgr.corpus {
		for _, inp := range cov.Cover {
			if _, ok := uniqueCovers[inp]; ok {
				continue
			}
			uniqueCovers[inp] = true
			buf = append(buf, []byte(strconv.FormatUint(uint64(inp), 10))...)
			buf = append(buf, '\n')
		}
	}
	ioutil.WriteFile("cover-"+timestamp, buf, 0777)
}
