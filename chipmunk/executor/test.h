...
static int test_csum_inet()
{
	...
	struct csum_inet_test tests_inet[] = {
	    {// 0
	     "",
	     0,
	     le16toh(0xffff)},
	...
	for (unsigned i = 0; i < ARRAY_SIZE(tests_inet); i++) {
		struct csum_inet csum;
		csum_inet_init(&csum);
		csum_inet_update(&csum, (const uint8*)tests_inet[i].data, tests_inet[i].length);
		if (csum_inet_digest(&csum) != tests_inet[i].csum) {
			fprintf(stderr, "bad checksum in test #%u, want: %hx, got: %hx\n", i, tests_inet[i].csum, csum_inet_digest(&csum));
			return 1;
		}
	}
...
{
	int ret = 0;
	for (size_t i = 0; i < ARRAY_SIZE(tests_basic); i++) {
		const char* name = tests_basic[i].name;
		printf("=== RUN  %s\n", name);
		int res = tests_basic[i].f();
		ret |= res > 0;
