    ...
    namespace tests {
        ...
        class testName: public BaseTestCase {
            ...
            virtual int run( int checkpoint ) override {
				...
				if ( cm_->CmFallocate( fd_Afoo , FALLOC_FL_KEEP_SIZE , 4096 , 4096) < 0){ 
					cm_->CmClose( fd_Afoo);
					 return errno;
				}
				...
				void* data_foo;
				if (posix_memalign(&data_foo , 4096, 1024 ) < 0) {
					return errno;
				}

				 
				int offset_foo = 0;
				int to_writefoo = 1024 ;
				const char *text_foo  = "ddddddddddklmnopqrstuvwxyz123456";
				while (offset_foo < 1024){
					if (to_writefoo < 32){
				...
				if ( cm_->CmPwrite ( fd_foo, data_foo, 1024, 0) < 0){
					cm_->CmClose( fd_foo); 
					return errno;
				}
