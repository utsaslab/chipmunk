...
class DiskMod {
 public:
  ...
  static char* Serialize(DiskMod &dm, unsigned long long *size);
  ...
  enum ModOpts {
    ...
    kTruncateOpt,           // truncate with truncate() or ftruncate()
    kTruncateOpenOpt,       // truncate on open
    // kChmodOpt,              // changing permissions operation
    kWriteOpt,              // distinguish between write and pwrite calls to help 
    kPwriteOpt,             // match stuff up with the system call record
  ...
  // TODO: handle mode and flags better so they aren't recorded when a system call
  // doesn't use them

  std::string path;
  int return_value;
  mode_t mode; // ignored if the mod doesn't change the mode
  int flags; // ignored if the mod isn't open
  int fd = -1;
  ModType mod_type;
  ModOpts mod_opts;
  struct stat post_mod_stats;
  bool path_mod; // did this modification involve more than one path (e.g. rename, link)?
  std::string new_path;
 ...
 private:
  ...
  // void deserialize_string(std::string &path, char* data_ptr);
  ...
  static int SerializeDirectoryMod(char *buf, const unsigned int len,
      DiskMod &dm);
  static int SerializeNewPath(char *buf, const unsigned int len, 
      DiskMod &dm);
};
