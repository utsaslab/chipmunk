...
func (cfg *Config) completeBinaries() error {
	...
	targetLogger := func(name, arch string) string {
		return filepath.Join(cfg.Syzkaller, "bin", cfg.TargetOS+"_"+arch, "loggers", name)
	}
	...
	cfg.ExecutorCCBin = targetBin("syz-executor_cc", cfg.TargetArch)

	cfg.Tester = strings.ToLower(cfg.Tester)
	cfg.AceTesterBin = targetBin("ace-executor_cc", cfg.TargetArch)
	cfg.AceTestDir = targetBin("tests/seq1/", cfg.TargetArch)
	...
	} else if FileSystem == "pmfs" {
		// return fmt.Errorf("pmfs is not yet supported")
