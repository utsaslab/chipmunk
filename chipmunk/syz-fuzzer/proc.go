...
func (proc *Proc) loop() {
	...
	for i := 0; ; i++ {
		...
		if item != nil {
			...
			case *WorkTriage:
				log.Logf(0, "TRIAGING WORKQUEUE")
				proc.triageInput(item)
			case *WorkCandidate:
				log.Logf(0, "EXECUTING WORKQUEUE")
				proc.execute(proc.execOpts, item.p, item.flags, StatCandidate)
			case *WorkSmash:
				log.Logf(0, "SMASH WORKQUEUE")
...
func (proc *Proc) triageInput(item *WorkTriage) {
	...
	if item.p.Filter() {
		log.Logf(1, "filtering item: %v", item.p.Serialize())
		return
	}
	...
	for i := 0; i < signalRuns; i++ {
		log.Logf(0, "EXECUTE RAW SIGNAL RUNS")
...
func getSignalAndCover(p *prog.Prog, info *ipc.ProgInfo, call int) (signal.Signal, []uint32) {
	...
	if call != -1 {
		...
		inf.Signal = info.MountSignal
		inf.Cover = info.MountCover
	}
...
func (proc *Proc) smashInput(item *WorkSmash) {
	if item.p.Filter() {
		log.Logf(0, "Filtering smash: %v", item.p.Serialize())
	}
	if proc.fuzzer.faultInjectionEnabled && item.call != -1 {
		log.Logf(0, "FAULT INJECTING FAIL CALL")
		proc.failCall(item.p, item.call)
	}
	if proc.fuzzer.comparisonTracingEnabled && item.call != -1 {
		log.Logf(0, "HINT SEED COMPARISON")
...
func (proc *Proc) execute(execOpts *ipc.ExecOpts, p *prog.Prog, flags ProgTypes, stat Stat) *ipc.ProgInfo {
	log.Logf(0, "execute func: opts: %v", *execOpts)
	if p.Filter() {
		log.Logf(1, "Filtering: %v", p.Serialize())
		return nil
	}
	info := proc.executeRaw(execOpts, p, stat)
	if info == nil {
		log.Logf(0, "INFO IS NIL")
		return nil
	}
	calls, extra := proc.fuzzer.checkNewSignal(p, info)
	//log.Logf(0, "GOING TO TRIAGE CALLS WITH INFO: %v %v", info, proc)
	...
	if execOpts.ConsistencyCheck && execOpts.CollectMountCov {
		proc.enqueueCallTriage(p, flags, -1, info.Extra)
	}
	return info
}
...
func (proc *Proc) executeRaw(opts *ipc.ExecOpts, p *prog.Prog, stat Stat) *ipc.ProgInfo {
	...
	for try := 0; ; try++ {
		...
		log.Logf(0, "EXECUTING PROG")
		output, info, hanged, err := proc.env.Exec(opts, p, proc.fuzzer.name)
		if err != nil {
