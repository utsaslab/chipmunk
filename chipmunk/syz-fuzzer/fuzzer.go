...
func main() {
	...
	var (
		...
		flagCrashConsistency = flag.Bool("crashConsistency", true, "test for crash consistency")
		flagCCTrackMount     = flag.Bool("ccmount", false, "capture coverage while mounting")
		flagRestart          = flag.Bool("restart", false, "restart executor after each run")
		flagLogger           = flag.String("logger", "/logger-nova.ko", "path to fs logger")
		flagFS               = flag.String("fs", "nova", "file system to test")
		flagModule           = flag.Bool("module", false, "load fs as module")
		flagReload           = flag.Bool("reload", false, "reload filesystem after each run")
		flagFSPath           = flag.String("fs_path", "/nova.ko", "file system module path")
	)
	defer tool.Init()()
	...
	config.CheckConsistency = *flagCrashConsistency
	config.CollectMountCov = *flagCCTrackMount
	config.FSLogger = *flagLogger
	config.FS = *flagFS
	config.Module = *flagModule
	config.Reload = *flagReload
	config.FSPath = *flagFSPath
	execOpts.ConsistencyCheck = *flagCrashConsistency
	...
	if r.CheckResult == nil {
		...
		if *flagCrashConsistency {
			execBin = config.ExecutorCC
		} else {
			execBin = config.Executor
		}
		if err = host.Setup(target, r.CheckResult.Features, featureFlags, execBin); err != nil {
			log.Fatal(err)
		}
	}
	...
	if *flagCrashConsistency {
		fuzzer.setupHostshare()
		// logger is copied into root directory
	}
...
func (fuzzer *Fuzzer) setupHostshare() {
	out, err := exec.Command("mount", "-t", "9p", "-o", "trans=virtio,version=9p2000.L", "hostshare", "/root/tmpdir/").Output()
	if err == nil {
		log.Logf(1, "OUT: %v", string(out))
	} else {
		log.Fatalf("FAILED TO SETUP HOSTSHARE: %v", err)
	}
}
