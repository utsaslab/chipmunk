...
var (
	...
	flagRestart   = flag.Bool("restart", false, "restart executor after every test")
	flagFS        = flag.String("fs", "nova", "file system  to fuzz")
	flagModule    = flag.Bool("module", false, "load file system as module")
	// flagLogger           = flag.String("logger", "/root/tmpdir/nova-tester/build/logger-nova.ko", "path to logger")
	flagLogger           = flag.String("logger", "/root/syzkallerBinaries/linux_amd64/loggers/logger-nova.ko", "path to logger")
	flagFSPath           = flag.String("fs_path", "/root/tmpdir/linux-5.1/fs/nova/nova.ko", "path to file system module")
	flagReload           = flag.Bool("reload", false, "reload file system after every test")
	flagCrashConsistency = flag.Bool("crashConsistency", false, "crash consistency")
	flagMountCov         = flag.Bool("mountCov", false, "crash consistency")
...
func main() {
	...
	if *flagReload && !*flagModule {
		log.Fatalf("config reload requires module")
	}
	target, err := prog.GetTarget(*flagOS, *flagArch)
	if err != nil {
		log.Fatalf("%v", err)
	}
	// files := readCrashDirectory("/root/tmpdir/nova-tester/lastRun/")
	// log.Logf(0, "%v", files)

	// test_index := 0
	// tests_num := 100
	// for i := 0; i < len(files); i++ {
	// 	// log.Logf(0, "%v, %v", files[i], i)
	// 	if files[i] == "16a999a4bcd8d064" {
	// 		test_index = i
	// 	}
	// }

	// files = files[test_index-tests_num : test_index+1]

	entries := loadPrograms(target, flag.Args())
	// entries := loadPrograms(target, files)
	if len(entries) == 0 {
		return
	}
	// for _, entry := range entries {
	// 	// log.Logf(0, "ENTRY CHECK: %v", entry.P.Checkpoint)
	// }
	...
	config.Reload = *flagReload
	config.FS = *flagFS
	config.Module = *flagModule
	config.FSLogger = *flagLogger
	config.FSPath = *flagFSPath
	execOpts.Restart = *flagRestart
...
func readCrashDirectory(dirPath string) []string {
	files, err := ioutil.ReadDir(dirPath)
	if err != nil {
		log.Fatalf("failed reading directory %v: %e", dirPath, err)
	}
...
func loadModule(path string) {
	log.Logf(1, "LOADING MODULE: %s", path)
	out, err := exec.Command("insmod", path).Output()

	//err := cmd.Run()

	if err != nil {
		log.Logf(1, "OUTPUT: %s", string(out))
		log.Fatalf("FAILED TO LOAD MODULE: %v\n", err)
	}
	log.Logf(1, "Loaded module: %s", string(out))
}
...
func (ctx *Context) execute(pid int, env *ipc.Env, entry *prog.LogEntry) {
	...
	// log.Logf(0, "ENTRY CHECKPOINT: %v", entry.P.Checkpoint)
	// This mimics the syz-fuzzer logic. This is important for reproduction.
	for try := 0; ; try++ {
		log.Logf(0, "RUNNING EXEC")
		output, info, hanged, err := env.Exec(callOpts, entry.P, "test")
		log.Logf(0, "DONE RUNNING EXEC")
		//log.Logf(0, "INFO MOUNT SIGNAL: %v", info)
...
func (ctx *Context) dumpCoverage(coverFile string, info *ipc.ProgInfo) {
	...
	buf := new(bytes.Buffer)
	if *flagCrashConsistency && *flagMountCov {
		fmt.Fprintf(buf, "Mount Signal\n")
		for _, pc := range info.MountCover {
			fmt.Fprintf(buf, "0x%x\n", cover.RestorePC(pc, 0xffffffff))
		}
	}
	osutil.WriteFile(coverFile+"_mount", buf.Bytes())
...
func loadPrograms(target *prog.Target, files []string) []*prog.LogEntry {
	log.Logf(0, "loading programs")
	var entries []*prog.LogEntry
	for _, fn := range files {
		fn_path := "/root/tmpdir/crashConsistencyProgs/" + fn
		log.Logf(0, "%v", fn_path)
		data, err := ioutil.ReadFile(fn_path)
		if err != nil {
			log.Fatalf("failed to read log file: %v", err)
		}
		newEntry := target.ParseLog(data)
		(*newEntry[0]).P.Name = fn
...
func createConfig(target *prog.Target, features *host.Features, featuresFlags csource.Features) (
	*ipc.Config, *ipc.ExecOpts) {
	...
	if *flagCrashConsistency {
		execOpts.ConsistencyCheck = true
		config.CheckConsistency = true
	}
	if *flagMountCov {
		execOpts.CollectMountCov = true
		config.CollectMountCov = true
	}
