...
static int pmfs_init_blockmap_from_inode(struct super_block *sb)
{
	...
	pmfs_dbg("total_blocknodes = %lu, num_blocks = %lu, num_free_blocks = %lu, blocknode mapping file size = %lu\n",
		 total_blocknodes, sbi->num_blocks, sbi->num_free_blocks, pi->i_size);
...
static int pmfs_init_inode_list_from_inode(struct super_block *sb)
{
	...
	for (block_ctr = 0; block_ctr < pi->i_size / sbi->blocksize; block_ctr++) {
		...
		for (inode_node_ctr = 0; inode_node_ctr < RANGENODE_PER_PAGE; inode_node_ctr++) {
			...
			if (blocknode_ctr == total_inode_nodes)
				break;
    ...
    for (i = 0; i < sbi->cpus; i++) {
		...
		if (!inode_map->first_inode_range) {
			new_range_node = pmfs_alloc_inode_node(sb);
			new_range_node->range_low = 0;
			new_range_node->range_high = 0;
			ret = pmfs_insert_inodetree(sbi, new_range_node, i);
			if (ret) {
				pmfs_err(sb, "%s failed\n", __func__);
				pmfs_free_inode_node(sb, new_range_node);
				PMFS_ASSERT(0);
				pmfs_destroy_inode_trees(sb);
				goto out;
			}
        ...
        pmfs_dbg("inode_map->first_inode_range->range_low = %lu, "
                "inode_map->first_inode_range->range_high = %lu\n",
                inode_map->first_inode_range->range_low,
                inode_map->first_inode_range->range_high);
    }
...
void pmfs_save_blocknode_mappings(struct super_block *sb)
{
	...
	pmfs_dbg("%s: num blocknodes to save = %lu. num pages = %lu, num_blocknode_allocated = %lu\n",
		 __func__, num_blocknode, num_pages, sbi->num_blocknode_allocated);
...
static int alloc_bm(struct super_block *sb, unsigned long initsize, unsigned long initsize_2)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		bm->scan_bm_4K.bitmap = kvzalloc(bm->scan_bm_4K.bitmap_size, GFP_KERNEL);
		// bm->scan_bm_2M.bitmap = vzalloc(bm->scan_bm_2M.bitmap_size);
		bm->scan_bm_2M.bitmap = kvzalloc(bm->scan_bm_2M.bitmap_size, GFP_KERNEL);
		// bm->scan_bm_1G.bitmap = vzalloc(bm->scan_bm_1G.bitmap_size);
		bm->scan_bm_1G.bitmap = kvzalloc(bm->scan_bm_1G.bitmap_size, GFP_KERNEL);
...
int pmfs_recovery(struct super_block *sb, size_t size, size_t size_2)
{
	...
	if (value) {
		...
		if (ret)
			goto out;
		pmfs_dbg("finished allocating blockmap\n");
		sbi->s_inodes_used_count = 0;
		ret = pmfs_failure_recovery(sb);
		if (ret)
			goto out;
		pmfs_dbg("finished failure recovery\n");

		ret = pmfs_build_blocknode_map(sb, initsize, initsize_2);
		pmfs_dbg("finished building blocknode map\n");
	}
